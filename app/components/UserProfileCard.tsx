...
import {
  Cloud, Crown, Globe, Mail, MapPin, Phone, Star, User, X
} from "lucide-react";
import { useEffect, useState } from "react";
...
useEffect(() => {
  ...
  const ProfileModal = () => (
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full mx-4 overflow-hidden">
        <div className="relative bg-gradient-to-br from-blue-600 via-purple-600 to-indigo-700 px-6 py-8">
          <button onClick={() => setIsOpen(false)} className="absolute top-4 right-4 text-white hover:bg-white hover:bg-opacity-20 rounded-full p-2 transition-colors">
            <X className="w-5 h-5" />
          </button>
          <div className="text-center">
            <div className="w-20 h-20 rounded-full bg-white bg-opacity-20 backdrop-blur-sm flex items-center justify-center mx-auto mb-4 border-4 border-white border-opacity-30">
        ...
        <div className="p-6 space-y-6">
          <div className="space-y-4">
            <UserInfo label="Username" value={userData.username} icon={<User className="w-5 h-5 text-blue-600" />} />
            <UserInfo label="Email" value={userData.email} icon={<Mail className="w-5 h-5 text-green-600" />} />
            <UserInfo label="Subscription" value={userData.subscription} icon={<Crown className="w-5 h-5 text-purple-600" />} badge="PRO" />
          </div>

          <div className="border-t pt-6">
            <SectionHeader icon={<Globe className="w-4 h-4" />} title="Location Details" />
            <UserDetail label="Country" value={userData.country} />
            <UserDetail label="State" value={userData.state} />
          </div>
  ...
  const UserInfo = ({ label, value, icon, badge }: any) => (
    <div className="flex items-center gap-3">
  ...
  const UserDetail = ({ label, value }: any) => (
